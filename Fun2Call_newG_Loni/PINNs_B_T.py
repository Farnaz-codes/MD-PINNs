...
from Fun2Call_newG_Loni import NonDim_param_symmetry_3Blood_tumor_newG_Pvar as NDparam
from Fun2Call_newG_Loni import PINN_model_G
...
dTb_Tt = 2/8  
#P0 = 0.75 # constant P, if P is constant   P0  W/cm
P_range = [0.5,1.0]
...
class PINNs_tf2_Power():
  ...
  def __init__(self, input_args, Tissues_name, layers, lb, ub, blood_band,
               ...
               act_func = 'tanh', opt='adam', path_blood=[], PDEw = 100,tn = 0):
    # 1) inputs
    ...
    if self.load_w_t==False:
        ...
        Loss_num_t =  21 + 31  + 6 if self.Is_tumor else 21 #
    ...
    for key in self.Tissues_name: #self.Model_keys:
        ...
        model_T = PINN_model_G.PINN_model(act_func, layers['Skin'], LBUB=(lb[key],ub[key]))
    ...
    self.optimizer_Wt = tf.keras.optimizers.SGD(learning_rate = 0.99)
  ...
  def Tissues_DataSet(self,args_tissue = [], rank_ = 0,data_path=[]):       # Tissues
     ...
     if self.load_data:
         with open(data_path+'/newData_MCMH'+str(rank_)+'_Rank.pkl' , 'rb') as f:
             new_data = pkl.load(f)
         T0 ,T0t = self.create_T0(new_data['X_tissue'][0], self.ntime)
         self.train_dataset = tf.data.Dataset.from_tensors(new_data['X_tissue'][:-2]+(T0,T0t))
             
     else:
        # X0, ,Xf, X_ub, X_lb, Y_lb, Z_lb [all tissues (7)], 
        # Z_ub [skin_3rd, tumor2, gold, skin_3rd_blood]
        X0_p = self.fd(args_tissue['X0'])
        T0, T0t = self.create_T0(X0_p, self.ntime)
         
        dataset = (X0_p, self.fd(args_tissue['Xf']),  self.fd(args_tissue['X_lb']),
                        self.fd(args_tissue['X_ub']), self.fd(args_tissue['Y_lb_ub']),
                        self.fd(args_tissue['Z_lb']), self.fd(args_tissue['Z_ub']),T0 ,T0t)
  ...
  def create_T0(self,x0, ntime):
     if self.Time[0]>0: 
        input_0  = self.add_ConstColumn(self.ft(x0), C = ntime, NCol = 3)

        with tf.GradientTape(persistent=True) as tape:
            tape.watch(input_0)
            T0 = self.merged_model_T0(inputs = input_0) # T is a tuple with size of 3                
        ...
        ut_0 = tuple(tf.zeros(xi.shape[:-1]+(1,),dtype=DTYPE) for xi in x0)
     return  u_0, ut_0
  #----------------------------------------------------------------------------#
  ...
  def Loss_Skin_tissues(self, X_dist_batch): 
     ...
     Loss_0 = self.Skin_Loss_IC(self.ft(x0), T00, T0t) #--> Nt losses
  ...
  def Skin_Loss_IC(self,x0, T00, T0t =[], plt_loss=0): 
  ...
  def Skin_Loss_BCZ(self, z_lb, z_ub, plt_loss=0):  # Z is 4D (x,y,z,t,p) 
      ...
      #-----------------------------------------------------------------------# 
      w = [10,8,10,4,7,6,6,6] if self.Is_dTsquare else [7,7,4,1]
  ...
  def Skin_Loss_PDE(self, x  ,plt_loss = 0): 
    ...
    wf = self.PDEw if self.Is_tumor else [20, 20, 10]#wf = [2,2,6,8]
 ...
 #%% *********************** Blood Vessels Losses ****************************#
  ...
  def main_Metropolis_Hasting_Alg(self, Xt ): 
      ...
      x0, T0, T0t = self.Metropolis_Hasting_Alg_IC(x0,T0, T0t)
    ...
    while stopping_run<=10002 and it<it_Stop :
      ...
      if it>self.it_Stop*0.92 or stopping_run>0: self.max_lr, self.base_lr = 1, 1
         ...
         if self.LR_method =='cycle':
            ...
            elif it>self.it_Stop*0.82: self.max_lr = 0.5  # 8e-6
            elif it>self.it_Stop*0.74: self.max_lr = 0.25 # 1e-5
         else:
  ...
  #---------------------------------------------------------------------------#
  def Metropolis_Hasting_Alg_IC(self, X0, T00, T00t):
      X0, T00, T00t = self.ft(X0),self.ft(T00,1), self.ft(T00t,1)
      Loss_0_data = self.Skin_Loss_IC(X0, T00 , T00t, plt_loss = 1)
      ...
      T0p, T0t_p = self.create_T0(X0_prop, self.ntime)
      #-----------------------------------------------------------------------#
      ...
      #-----------------------------------------------------------------------#
      Loss_0_prop = self.Skin_Loss_IC(X0_prop, T0p,T0t_p, plt_loss = 1)
      ...
      T0_new, T0t_new = self.create_T0(X0new, self.ntime)
